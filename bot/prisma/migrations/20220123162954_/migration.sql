/*
  ...
  - Added the required column `serverId` to the `Poll` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE "Poll" ADD COLUMN     "serverId" TEXT NOT NULL;
...
-- AddForeignKey
ALTER TABLE "Poll" ADD CONSTRAINT "Poll_serverId_fkey" FOREIGN KEY ("serverId") REFERENCES "Server"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
