/*
  ...
  - Added the required column `channelId` to the `Stopwatch` table without a default value. This is not possible if the table is not empty.
  - Added the required column `serverId` to the `Stopwatch` table without a default value. This is not possible if the table is not empty.
  - Added the required column `userId` to the `Stopwatch` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE "Stopwatch" ADD COLUMN     "channelId" TEXT NOT NULL,
...
-- AddForeignKey
ALTER TABLE "Stopwatch" ADD CONSTRAINT "Stopwatch_userId_fkey" FOREIGN KEY ("userId") REFERENCES "User"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
