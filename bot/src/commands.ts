...
const filenames = fs.readdirSync(path.join(__dirname, './commands')).filter(f => (
                                                            f.endsWith('.ts') && !f.endsWith('.test.ts') && !f.endsWith('.sub.ts')
                                                         || f.endsWith('.js') && !f.endsWith('.test.js') && !f.endsWith('.sub.js')));
let COMMANDS: any = {};
try {
    filenames.forEach((f) => {
